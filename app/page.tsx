    ...
    <main className="p-2 max-w-[1920px] mx-auto">
      <div className="grid sm:grid-cols-4 md:grid-cols-8 xl:grid-cols-12 gap-2 md:gap-4 lg:gap-6 lg:px-4 px-2">
        ...
        <div className="col-span-4 xl:col-span-8 grid gap-2 md:gap-4 lg:gap-6 grid-cols-4 xl:grid-cols-8">
          ...
          <motion.section 
            className="flex flex-col col-span-4 sm:col-span-2 md:col-span-4 space-y-4 lg:space-y-6"
            initial={isMobile ? "hidden" : "visible"}
            whileInView={isMobile ? "visible" : "visible"}
            viewport={{ once: true, margin: "-20px" }}
            variants={sectionVariants}
        ...
        >
          <div className="bg-white/25 backdrop-blur-sm dark:bg-slate-600/25 dark:border-purple-950/25 shadow-md border rounded p-5 h-full">
            ...
            <Link target="_blank" href={allLinks.connectLink.href} className="nav-btn inline-flex mt-4">
              {allLinks.connectLink.label} <MessageSquare className="w-[15px] ms-1"/>
            </Link>
          </div>
        </motion.section>
      </div>
